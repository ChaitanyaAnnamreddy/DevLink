...
const Hero = () => {
  ...
  // Get the first user from the feed
  const currentUser = selector?.feed?.data?.[0]

  const handleSendRequest = async (status, userId, firstName, lastName) => {
    if (!userId) {
      return
    }
    ...
    try {
      ...
      setNotification({
        ...
        message:
          status === 'interested'
            ? `🎉 You showed interest in ${firstName} ${lastName}! Let's see if they accept! 🚀`
            : `🙅 You skipped ${firstName} ${lastName}. We'll find more matches for you soon! 💡`,
      })
  ...
  const getFeed = async () => {
    try {
      ...
      if (error.response?.status === 401) {
        ...
        navigate('/error', {
          state: { message: error.response?.data || 'Something went wrong!' },
        })
      }
    }
  }
  ...
  return (
    <>
      <div
        className="h-screen flex gap-8 items-center overflow-y-auto px-4 md:px-36 lg:px-16 py-24 scrollbar-smooth"
        data-theme="retro"
      >
        ...
        {currentUser ? (
          ...
          >
            ...
            <div className="card-body flex flex-col justify-center">
              ...
              <p className="flex justify-center">{currentUser?.about}</p>
          ...
          <div className="flex flex-wrap justify-center mx-4 md:mx-auto">
            ...
            <p className="text-3xl w-full flex justify-center">
              🚀 No new developers to show. Check back later
            </p>
          </div>
        )}
      </div>
